...
{
    internal class EfsStoreLockingManager : ILockingManager
    {
        ...
        private class ItemLockList : List<EfsStoreLockingManager.ItemLockInfo>
        {
        }

        private class ItemLockTypeDictionary : Dictionary<LockType, EfsStoreLockingManager.ItemLockList>
        {
        }

        private const string TokenScheme = "opaquelocktoken";

        private readonly IDictionary<string, EfsStoreLockingManager.ItemLockTypeDictionary> _itemLocks = new Dictionary<string, EfsStoreLockingManager.ItemLockTypeDictionary>();

        private static readonly LockEntry[] SupportedLocks =
        {
        ...
        public LockResult Lock(IStoreItem item, LockType lockType, LockScope lockScope, XElement owner, Uri lockRootUri, bool recursive, IEnumerable<int> timeouts)
        {
            ...
            lock (_itemLocks)
            {
                ...
                if (!_itemLocks.TryGetValue(key, out var itemLockTypeDictionary))
                    _itemLocks.Add(key, itemLockTypeDictionary = new EfsStoreLockingManager.ItemLockTypeDictionary());
                ...
                if (!itemLockTypeDictionary.TryGetValue(lockType, out var itemLockList))
                {
                    ...
                    itemLockTypeDictionary.Add(lockType, itemLockList = new EfsStoreLockingManager.ItemLockList());
                }
                else
                {
                ...
                var itemLockInfo = new EfsStoreLockingManager.ItemLockInfo(item, lockType, lockScope, lockRootUri, recursive, owner, timeout ?? -1);
        ...
        private static ActiveLock GetActiveLockInfo(EfsStoreLockingManager.ItemLockInfo itemLockInfo)
        {
